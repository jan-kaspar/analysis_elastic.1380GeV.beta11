...
void DoMatch(const string &label, TGraph *g, double th_y_min, double th_y_max, FILE *f_res_out, const string &res_label)
{
	...
	for (double sh = sh_min; sh <= sh_max; sh += sh_step)
	{
		...
		double norm = double(n_entries) / GaussIntegral(th_y_min_sh, th_y_max_sh, si_th_x);
		double sum = 0.;
		double max_diff = -1.;
		for (const auto &p : ch_th_y.data)
		{
			...
			double F = GaussIntegral(th_y_min_sh, th_y, si_th_x) * norm;
...
int main(int argc, const char **argv)
{
	...
	// prepare output
	TFile *f_out = TFile::Open((outputDir + "/match_th_y_scale_fcn.root").c_str(), "recreate");

	FILE *f_res_out = fopen((outputDir + "/match_th_y_scale_fcn.out").c_str(), "w");
