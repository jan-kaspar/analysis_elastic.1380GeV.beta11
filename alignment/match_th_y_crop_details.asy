...
for (int diri : dirs.keys)
{
	...
	draw(scale(1e6, 1), RootGetObject(f, base + "/cmp|h_th_x"), "vl", blue);
	draw(scale(1e6, 1), RootGetObject(f, base + "/cmp|h_th_y"), "vl", red);
...
for (int diri : dirs.keys)
{
	...
	string f = topDir + "match_th_y_crop.root";
	string base = dataset + "/" + diagonal + "/" + rp + "/" + dirs[diri];
	draw(scale(1e6, 1), RootGetObject(f, base + "/g_ch_th_x"), "l", blue);
	draw(scale(1e6, 1), RootGetObject(f, base + "/g_ch_th_y"), "l", red);
