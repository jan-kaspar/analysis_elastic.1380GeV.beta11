...
int main(int argc, char **argv)
{
	...
	for (int pi = 0; pi < g_idx->GetN(); ++pi)
	{
		...
		for (unsigned int ui = 0; ui < units.size(); ui++)
		{
			fprintf(fo, ",%E,%E,%E", graphs[ui].a->Eval(center), graphs[ui].b->Eval(center), 0.);
			// TODO
			//fprintf(fo, ",%E,%E,%E", graphs[ui].a->Eval(center), graphs[ui].b->Eval(center), graphs[ui].c->Eval(center));
		}
		fprintf(fo, "\n");
	}
	fclose(fo);
